...
- 新安装成功后，安装器会打印形如 `/login#setup-token=...` 的首次初始化链接。该令牌只用于创建第一个管理员，请像密码一样保管；页面通过请求头提交并在成功后清理地址栏 fragment。
- `subboost update` 会先校验候选配置并拉取镜像，再暂停旧实例、创建并验证 PostgreSQL 备份，候选健康后才切换正式文件。候选失败会恢复旧文件、旧镜像和数据库。
- 如果数据库恢复本身失败，应用和定时任务会保持停止，备份不会删除。此时不要强制重启，请按命令输出的恢复路径处理；可先运行 `subboost doctor` 和 `subboost logs` 收集状态。
